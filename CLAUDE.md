...
- `cmd/keelage-server`: `serve --db --key [--github-client-id] [--github-app-id --github-app-key --github-webhook-secret] [--checkpoint-interval]` · `org create|list|link-installation` · `member add` · `token create|revoke` · `checkpoint <org>` · `key`. 체크포인트 워커는 움직인 org만 서명한다.
- 릴리스(ADR 0016): `.goreleaser.yaml`(linux/darwin × amd64/arm64, checksums, syft SBOM, cosign keyless 번들, `ghcr.io/young1ll/keelage-server` 이미지) ← `.github/workflows/release.yml`(`v*` 태그). `scripts/install.sh`(체크섬 + cosign 검증), `deploy/docker/{Dockerfile,compose.yaml}`. 로컬 확인: `goreleaser check`, `goreleaser release --snapshot --clean --skip=docker,sign,sbom,publish`.
- 검증: `make lint test` · `make check-generated`(스키마 생성물 diff) · `make test-pg`(일회용 Postgres 16, `scripts/pg-test.sh`; 마이그레이션을 고쳤으면 `make pg-reset`) · CI `vuln`(govulncheck v1.1.4 고정) · `cmd/keelage/install_test.go`(가짜 릴리스로 install.sh). 사람 검증은 `docs/verification-runbook.md`, 외부 사용자 표는 `docs/metrics/external-users.md`. dogfood: `make dogfood-record` → 커밋마다 `make dogfood-verify`(docs/metrics).
